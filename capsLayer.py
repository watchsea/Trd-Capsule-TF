...
def routing(input, b_IJ):
    ...
    W = tf.get_variable('Weight', shape=(1, 1152, label_num, 8, 16), dtype=tf.float32,
    ...
    input = tf.tile(input, [1, 1, label_num, 1, 1])
    W = tf.tile(W, [cfg.batch_size, 1, 1, 1, 1])
    assert input.get_shape() == [cfg.batch_size, 1152, label_num, 8, 1]

    # in last 2 dims:
    # [8, 16].T x [8, 1] => [16, 1] => [batch_size, 1152, 10, 16, 1]
    u_hat = tf.matmul(W, input, transpose_a=True)
    assert u_hat.get_shape() == [cfg.batch_size, 1152, label_num, 16, 1]
    ...
    for r_iter in range(cfg.iter_routing):
        with tf.variable_scope('iter_' + str(r_iter)):
            ...
            assert c_IJ.get_shape() == [cfg.batch_size, 1152, label_num, 1, 1]
            ...
            assert s_J.get_shape() == [cfg.batch_size, 1, label_num, 16, 1]

            # line 6:
            # squash using Eq.1,
            v_J = squash(s_J)
            assert v_J.get_shape() == [cfg.batch_size, 1, label_num, 16, 1]
            ...
            assert u_produce_v.get_shape() == [cfg.batch_size, 1152, label_num, 1, 1]
